    ...
    public class TaikoSelectionHandler : SelectionHandler
    {
        ...
        protected override IEnumerable<MenuItem> GetContextMenuItemsForSelection(IEnumerable<SelectionBlueprint> selection)
        {
            if (selection.All(s => s.HitObject is Hit))
                yield return new TernaryStateMenuItem("咔") { State = { BindTarget = selectionRimState } };

            if (selection.All(s => s.HitObject is TaikoHitObject))
                yield return new TernaryStateMenuItem("大音符") { State = { BindTarget = selectionStrongState } };
        }
