...
namespace osu.Game.Rulesets.Osu.Tests.Editor.Checks
{
    ...
    public class CheckLowDiffOverlapsTest
    {
        ...
        [Test]
        public void TestLowestNormal()
        {
            // Normal diff with issues, no others. Should fail.
            var beatmap = new Beatmap<HitObject>
            {
                ...
                BeatmapInfo = new BeatmapInfo { StarRating = 2.5 }
            };

            assertWithOthers(beatmap, DifficultyRating.Normal, System.Array.Empty<double>(), true);
        }

        [Test]
        public void TestNormalWithEasy()
        {
            // Normal diff with issues, but Easy exists. Should pass (skip).
            var beatmap = new Beatmap<HitObject>
            {
                HitObjects = new List<HitObject>
                {
            ...
            assertWithOthers(beatmap, DifficultyRating.Normal, new[] { 1.5 }, false);
        }

        [Test]
        public void TestNormalWithLowerNormal()
        {
            // Normal diff (2.5) with issues, lower Normal (2.2) exists. Should pass (skip).
            var beatmap = new Beatmap<HitObject>
            {
                HitObjects = new List<HitObject>
                {
            ...
            assertWithOthers(beatmap, DifficultyRating.Normal, new[] { 2.2 }, false);
        }

        [Test]
        public void TestNormalWithHigherNormal()
        {
            // Normal diff (2.2) with issues, higher Normal (2.5) exists. Should fail (run).
            var beatmap = new Beatmap<HitObject>
            {
                HitObjects = new List<HitObject>
                {
                ...
                BeatmapInfo = new BeatmapInfo { StarRating = 2.2 }
            };

            assertWithOthers(beatmap, DifficultyRating.Normal, new[] { 2.5 }, true);
        }

        private void assertWithOthers(IBeatmap current, DifficultyRating currentRating, IEnumerable<double> otherStarRatings, bool expectIssues)
        {
            var currentWorking = new TestWorkingBeatmap(current);
            if (current.BeatmapInfo.StarRating <= 0)
            ...
            var context = new BeatmapVerifierContext(
                new BeatmapVerifierContext.VerifiedBeatmap(currentWorking, current),
                otherVerified,
                currentRating
            );
