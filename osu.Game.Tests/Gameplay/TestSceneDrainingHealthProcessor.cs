...
namespace osu.Game.Tests.Gameplay
{
    ...
    public partial class TestSceneDrainingHealthProcessor : OsuTestScene
    {
        ...
        public void TestHealthGainedOnHit()
        {
            ...
            AddStep("apply hit result", () => processor.ApplyResult(new Judgement(beatmap.HitObjects[0], new JudgementInfo()) { Type = HitResult.Perfect }));
            assertHealthEqualTo(1);
        }
        ...
        public void TestHealthRemovedOnRevert()
        {
            ...
            createProcessor(beatmap);
            setTime(10); // Decrease health slightly
            AddStep("apply hit result", () => processor.ApplyResult(result = new Judgement(beatmap.HitObjects[0], new JudgementInfo()) { Type = HitResult.Perfect }));
        ...
        public void TestFailConditions()
        {
            ...
            AddStep("apply perfect hit result", () => processor.ApplyResult(new Judgement(beatmap.HitObjects[0], new JudgementInfo()) { Type = HitResult.Perfect }));
            AddAssert("not failed", () => !processor.HasFailed);
            AddStep("apply miss hit result", () => processor.ApplyResult(new Judgement(beatmap.HitObjects[0], new JudgementInfo()) { Type = HitResult.Miss }));
        ...
        public void TestMultipleFailConditions(HitResult resultApplied)
        {
            ...
            AddStep($"apply {resultApplied.ToString().ToLowerInvariant()} hit result",
                () => processor.ApplyResult(new Judgement(beatmap.HitObjects[0], new JudgementInfo()) { Type = resultApplied }));
            AddAssert("failed", () => processor.HasFailed);
        }
        ...
        private class JudgeableHitObject : HitObject
        {
            ...
            private class TestJudgementInfo : JudgementInfo
            {
                ...
                public TestJudgementInfo(HitResult maxResult)
                {
                    MaxResult = maxResult;
                }
            }
