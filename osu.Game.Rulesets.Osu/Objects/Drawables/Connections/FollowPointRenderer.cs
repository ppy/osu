            ...
            foreach (var currHitObject in hitObjects)
            {
                if (prevHitObject != null && !currHitObject.NewCombo && !(prevHitObject is Spinner) && !(currHitObject is Spinner))
                {
                    ...
                    for (int d = (int)(PointDistance * 1.5); d < distance - PointDistance; d += PointDistance)
                    {
                        ...
                        {
                            fp.FadeIn(DrawableOsuHitObject.TimeFadein);
                            fp.ScaleTo(1, DrawableOsuHitObject.TimeFadein, Easing.Out);

                            fp.MoveTo(pointEndPosition, DrawableOsuHitObject.TimeFadein, Easing.Out);

                            fp.Delay(fadeOutTime - fadeInTime).FadeOut(DrawableOsuHitObject.TimeFadein);
                        }
