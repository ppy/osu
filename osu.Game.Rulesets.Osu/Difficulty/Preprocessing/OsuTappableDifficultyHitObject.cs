...
{
    public class OsuTappableDifficultyHitObject : OsuDifficultyHitObject
    {
        /// <summary>
        ...
        private readonly IReadOnlyList<OsuTappableDifficultyHitObject> tappableDifficultyHitObjects;
        private readonly OsuHitObject tappableMainObject;

        protected new OsuHitObject BaseObject => (OsuHitObject)base.BaseObject;
        protected new OsuHitObject LastObject => (OsuHitObject)base.LastObject;

        /// <summary>
        /// The index of this <see cref="OsuTappableDifficultyHitObject"/> in the list of tappable <see cref="OsuTappableDifficultyHitObject"/>s.
        /// </summary>
        public int IndexTappable;

        /// <summary>
        /// <see cref="DifficultyHitObject.DeltaTime"/> between tappable objects.
        /// </summary>
        public readonly double DeltaTimeTappable;
        ...
        /// <summary>
        /// Normalised distance from the "lazy" end position of the previous <see cref="OsuTappableDifficultyHitObject"/> to the start position of this <see cref="OsuTappableDifficultyHitObject"/>.
        /// <para>
        ...
        /// <summary>
        /// Normalised shortest distance to consider for a jump between the previous <see cref="OsuTappableDifficultyHitObject"/> and this <see cref="OsuTappableDifficultyHitObject"/>.
        /// </summary>
        /// <remarks>
        /// This is bounded from above by <see cref="LazyJumpDistance"/>, and is smaller than the former if a more natural path is able to be taken through the previous <see cref="OsuTappableDifficultyHitObject"/>.
        /// </remarks>
        /// <example>
        ...
        /// <summary>
        /// Normalised distance between the start and end position of this <see cref="OsuTappableDifficultyHitObject"/>.
        /// </summary>
        ...
        /// <summary>
        /// The position of the cursor at the point of completion of this <see cref="OsuTappableDifficultyHitObject"/> if it is a <see cref="Slider"/>
        /// and was hit with as few movements as possible.
        /// </summary>
        public Vector2? LazyEndPosition { get; private set; }

        /// <summary>
        /// The distance travelled by the cursor upon completion of this <see cref="OsuTappableDifficultyHitObject"/> if it is a <see cref="Slider"/>
        /// and was hit with as few movements as possible.
        /// </summary>
        public double LazyTravelDistance { get; private set; }

        /// <summary>
        /// The time taken by the cursor upon completion of this <see cref="OsuTappableDifficultyHitObject"/> if it is a <see cref="Slider"/>
        /// and was hit with as few movements as possible.
        /// </summary>
        public double LazyTravelTime { get; private set; }

        /// <summary>
        /// Angle the player has to take to hit this <see cref="OsuTappableDifficultyHitObject"/>.
        ...
        private readonly OsuTappableDifficultyHitObject? lastLastDifficultyObject;
        private readonly OsuTappableDifficultyHitObject? lastDifficultyObject;

        public OsuTappableDifficultyHitObject(HitObject hitObject, HitObject lastObject, HitObject lastMainObject, double clockRate, List<DifficultyHitObject> objects, int index, List<OsuTappableDifficultyHitObject> tappableObjects, int tappableIndex)
            : base(hitObject, lastObject, clockRate, objects, index)
        {
            this.tappableMainObject = (OsuHitObject)lastMainObject;
            tappableDifficultyHitObjects = tappableObjects;
            IndexTappable = tappableIndex;

            lastLastDifficultyObject = index > 1 ? tappableObjects[tappableIndex - 2] : null;
            lastDifficultyObject = index > 0 ? tappableObjects[tappableIndex - 1] : null;

            DeltaTimeTappable = (hitObject.StartTime - lastMainObject.StartTime) / clockRate;

            // Capped to 25ms to prevent difficulty calculation breaking from simultaneous objects.
            AdjustedDeltaTime = Math.Max(DeltaTimeTappable, MIN_DELTA_TIME);
        ...
        public OsuTappableDifficultyHitObject? PreviousTappable(int backwardsIndex)
        {
            int index = IndexTappable - (backwardsIndex + 1);
            return index >= 0 && index < tappableDifficultyHitObjects.Count ? tappableDifficultyHitObjects[index] : default;
        }

        public OsuTappableDifficultyHitObject? NextTappable(int forwardsIndex)
        {
            int index = IndexTappable + (forwardsIndex + 1);
            return index >= 0 && index < tappableDifficultyHitObjects.Count ? tappableDifficultyHitObjects[index] : default;
        }
        ...
        /// </summary>
        public double GetDoubletapness(OsuTappableDifficultyHitObject? osuNextObj)
        {
            if (osuNextObj != null)
            {
        ...
        private void setDistances(double clockRate)
        {
            ...
            Vector2 lastCursorPosition = lastDifficultyObject != null ? getEndCursorPosition(lastDifficultyObject) : tappableMainObject.StackedPosition;

            LazyJumpDistance = (BaseObject.StackedPosition * scalingFactor - lastCursorPosition * scalingFactor).Length;
            MinimumJumpTime = AdjustedDeltaTime;
            MinimumJumpDistance = LazyJumpDistance;

            if (tappableMainObject is Slider lastSlider && lastDifficultyObject != null)
            {
            ...
            if (lastLastDifficultyObject != null && lastLastDifficultyObject.BaseObject is not Spinner)
            {
                ...
                Vector2 v1 = lastLastCursorPosition - tappableMainObject.StackedPosition;
        ...
        private Vector2 getEndCursorPosition(OsuTappableDifficultyHitObject difficultyHitObject)
        {
