    ...
    public class NoteVarianceAngle : PrePerNoteStrainSkill
    {
        ...
        protected override double StrainValueOf(DifficultyHitObject current)
        {
            ...
            if(Previous.Count > 0)
            {
                ...
                angleBonus = 0.01 * Math.Max(Math.Sin(osuCurrent.Angle.Value - angle_bonus_begin), 0);

                // 각도 변화 값
                // 각도가 자주 변화하면 보너스를 받도록 한다
                // bonus for changing angles frequently
                // 
                var angleVariance = Math.Sin(Math.Max(Math.Abs(angle - lastAngle) - Math.PI / 2, 0)) * 0.1;
