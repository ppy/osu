...
namespace osu.Game.Rulesets.Osu.Difficulty.Evaluators
{
    public static class AimEvaluator
    {
        ...
        private const double wiggle_multiplier = 1.92;
        ...
        public static double EvaluateDifficultyOf(DifficultyHitObject current, bool withSliderTravelDistance)
        {
            ...
            if (osuCurrObj.Angle != null && osuLastObj.Angle != null)
            {
                ...
                if (osuCurrObj.IsClockwise != osuLastObj.IsClockwise)
                {
                    // Apply wiggle bonus for jumps that are [radius, 3*diameter] in distance, with < 140 and > 5 angle
                    // https://www.desmos.com/calculator/dgmijkapzn
                    wiggleBonus = angleBonus
                                  * DifficultyCalculationUtils.Smootherstep(osuCurrObj.LazyJumpDistance, radius, diameter)
                                  * Math.Pow(DifficultyCalculationUtils.ReverseLerp(osuCurrObj.LazyJumpDistance, diameter * 5, diameter), 1.8)
                                  * DifficultyCalculationUtils.Smootherstep(currAngle, double.DegreesToRadians(5), double.DegreesToRadians(60))
                                  * DifficultyCalculationUtils.Smootherstep(currAngle, double.DegreesToRadians(140), double.DegreesToRadians(80))
                                  * DifficultyCalculationUtils.Smootherstep(osuLastObj.LazyJumpDistance, radius, diameter)
                                  * Math.Pow(DifficultyCalculationUtils.ReverseLerp(osuLastObj.LazyJumpDistance, diameter * 5, diameter), 1.8)
                                  * DifficultyCalculationUtils.Smootherstep(lastAngle, double.DegreesToRadians(140), double.DegreesToRadians(80))
                                  * DifficultyCalculationUtils.Smootherstep(lastAngle, double.DegreesToRadians(5), double.DegreesToRadians(60));
                }
