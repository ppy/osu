    ...
    public class OsuPerformanceCalculator : PerformanceCalculator
    {
        ...
        private double computeAimValue()
        {
            ...
            if (mods.Any(h => h is OsuModFlashlight))
            {
                ...
                double baseFlashlightMultiplier = 0.35 * Math.Min(1.0, totalHits / 200.0) +
                          (totalHits > 200
                            ? 0.3 * Math.Min(1.0, (totalHits - 200) / 300.0) +
                              (totalHits > 500 ? (totalHits - 500) / 1500.0 : 0.0)
                            : 0.0);
                ...
                baseFlashlightMultiplier *= Math.Pow(adjustedCombo / adjustedMaxCombo, 2.0);

                // Apply harsher nerf with misses.
                baseFlashlightMultiplier *= Math.Pow(0.97, countMiss);

                // Apply extra buff when Flashlight is combined with Hidden, based on Approach Rate.
                if (mods.Any(h => h is OsuModHidden))
                    baseFlashlightMultiplier *= 1.45 - 0.01 * Math.Pow(1.4, Attributes.ApproachRate);

                aimValue *= 1.0 + baseFlashlightMultiplier;
            }
