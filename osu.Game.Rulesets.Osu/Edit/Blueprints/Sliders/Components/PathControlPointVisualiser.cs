        ...
        where T : OsuHitObject, IHasPath
    {
        ...
        public MenuItem[] ContextMenuItems
        {
            get
            {
                ...
                {
                    menuItems.Add(new OsuMenuItem($"分离 {"个控制点".ToQuantity(splittableCount, splittableCount > 1 ? ShowQuantityAs.None : ShowQuantityAs.None)}",
                        MenuItemType.Destructive,
                        () => splitSelected()));
                }

                menuItems.Add(
                    new OsuMenuItem($"删除 {"个控制点".ToQuantity(count, count > 1 ? ShowQuantityAs.None : ShowQuantityAs.None)}",
                        MenuItemType.Destructive,
                        () => DeleteSelected())
                );
