    ...
    public class OsuRuleset : Ruleset, ILegacyRuleset
    {
        ...
        public override StatisticRow[] CreateStatisticsForScore(ScoreInfo score, IBeatmap playableBeatmap) => new[]
        {
            new StatisticRow
            {
                Columns = new[]
                {
                    new StatisticItem("误差", new HitEventTimingDistributionGraph(score.HitEvents.Where(e => e.HitObject is HitCircle && !(e.HitObject is SliderTailCircle)).ToList())
                    {
            ...
            new StatisticRow
            {
                Columns = new[]
                {
                    new StatisticItem("准确率分布", new AccuracyHeatmap(score, playableBeatmap)
                    {
                        RelativeSizeAxes = Axes.X,
                        Height = 250
                    }),
