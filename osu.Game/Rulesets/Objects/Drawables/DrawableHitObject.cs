...
namespace osu.Game.Rulesets.Objects.Drawables
{
    ...
    public abstract partial class DrawableHitObject : PoolableDrawableWithLifetime<HitObjectLifetimeEntry>, IAnimationTimeReference
    {
        ...
        protected void ApplyResult(Action<Judgement> application)
        {
            ...
            HitResultExtensions.ValidateHitResultPair(Result.JudgementInfo.MaxResult, Result.JudgementInfo.MinResult);

            if (!Result.Type.IsValidHitResult(Result.JudgementInfo.MinResult, Result.JudgementInfo.MaxResult))
            {
                throw new InvalidOperationException(
                    $"{GetType().ReadableName()} applied an invalid hit result (was: {Result.Type}, expected: [{Result.JudgementInfo.MinResult} ... {Result.JudgementInfo.MaxResult}]).");
            }
        ...
        protected virtual Judgement CreateResult(JudgementInfo judgementInfo) => new Judgement(HitObject, judgementInfo);
