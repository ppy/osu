...
namespace osu.Game.Rulesets.Objects.Legacy
{
    ...
    public abstract class ConvertHitObjectParser : HitObjectParser
    {
        ...
        public override HitObject Parse(ReadOnlySpan<char> text)
        {
            ...
            var posX = split.CurrentSpan;
            split.MoveNext(); // split[1]
            var posY = split.CurrentSpan;
            ...
            var soundType = (LegacyHitSoundType)Parsing.ParseInt(split.CurrentSpan);
            ...
            if (type.HasFlagFast(LegacyHitObjectType.Circle))
            {
                ...
                if (split.MoveNext()) // split[5]
                    readCustomSampleBanks(split.CurrentSpan, bankInfo);
            }
            else if (type.HasFlagFast(LegacyHitObjectType.Slider))
            {
                split.MoveNext(); // split[5]
                var pointSpan = split.CurrentSpan;
                ...
                var lengthSpan = split.CurrentSpan;
                split.MoveNext(); // split[8]
                var perNodeSound = split.CurrentSpan;
                split.MoveNext(); // split[9]
                var perNodeSample = split.CurrentSpan;
                split.MoveNext(); // split[10]
                var sampleBanks = split.CurrentSpan;
                ...
                {
                    int i = 0;
                    foreach (Range set in perNodeSample.Split('|'))
                    {
                        if (i >= nodes)
                            break;

                        SampleBankInfo info = nodeBankInfos[i];
                        readCustomSampleBanks(perNodeSample[set], info);
                        i++;
                    }
                }
                ...
                {
                    int i = 0;
                    foreach (Range add in perNodeSound.Split('|'))
                    {
                        if (i >= nodes)
                            break;

                        int.TryParse(perNodeSound[add], out int sound);
            ...
            {
                split.MoveNext(); // split[5]
                double duration = Math.Max(0, Parsing.ParseDouble(split.CurrentSpan) + Offset - startTime);
            ...
            else if (type.HasFlagFast(LegacyHitObjectType.Hold))
            {
                ...
                if (split.MoveNext()) // split[5]
                {
                    var ss = split.CurrentSpan.Split(':');
                    ss.MoveNext();
                    endTime = Math.Max(startTime, Parsing.ParseDouble(ss.CurrentSpan));
        ...
        private void readCustomSampleBanks(ReadOnlySpan<char> str, SampleBankInfo bankInfo)
        {
            ...
            split.MoveNext(); // split[0]
            var bank = (LegacySampleBank)Parsing.ParseInt(split.CurrentSpan);
            split.MoveNext(); // split[1]
            var addBank = (LegacySampleBank)Parsing.ParseInt(split.CurrentSpan);
            ...
            if (split.MoveNext()) // split[2]
                bankInfo.CustomSampleBank = Parsing.ParseInt(split.CurrentSpan);

            if (split.MoveNext()) // split[3]
                bankInfo.Volume = Math.Max(0, Parsing.ParseInt(split.CurrentSpan));

            bankInfo.Filename = split.MoveNext() // split[4]
                ? split.CurrentSpan.ToString()
                : null;
        }

        private static PathType convertPathType(ReadOnlySpan<char> input)
        {
            switch (input[0])
            {
                default:
        ...
        private struct PointOrPathType
        {
            // A union of Vector2|PathType, representing a segment in point string split by |
            public Vector2 Point;
            public PathType PathType;
            public bool IsPathType;

            public static PointOrPathType ParseFromPointString(ReadOnlySpan<char> input)
            {
                int splitIndex = input.IndexOf(':');
                if (splitIndex > 0)
                {
                    return new PointOrPathType
                    {
                        Point = new Vector2((int)Parsing.ParseDouble(input[..splitIndex], Parsing.MAX_COORDINATE_VALUE), (int)Parsing.ParseDouble(input[(splitIndex + 1)..], Parsing.MAX_COORDINATE_VALUE))
                    };
                }
                else
                {
                    return new PointOrPathType
                    {
                        PathType = convertPathType(input),
                        IsPathType = true
                    };
                }
            }
        ...
        private PathControlPoint[] convertPathString(ReadOnlySpan<char> pointString, Vector2 offset)
        {
            ...
            var pointSplitList = new List<PointOrPathType>();

            // LINQ is unavailable to ref struct
            foreach (var range in pointString.Split('|'))
                pointSplitList.Add(PointOrPathType.ParseFromPointString(pointString[range]));
            PointOrPathType[] pointSplit = pointSplitList.ToArray();
            ...
            {
                // Keep incrementing endIndex while it's not the start of a new segment (indicated by being a PathType).
                if (pointSplit[endIndex].IsPathType)
                    continue;
                ...
                var endPoint = endIndex < pointSplit.Length - 1 ? pointSplit[endIndex + 1] : (PointOrPathType?)null;
        ...
        private IEnumerable<Memory<PathControlPoint>> convertPoints(ReadOnlyMemory<PointOrPathType> points, PointOrPathType? endPoint, bool first, Vector2 offset)
        {
            if (!points.Span[0].IsPathType)
                throw new ArgumentException("The points to convert must start with a PathType.", nameof(points));
            PathType type = points.Span[0].PathType;
            ...
            if (endPoint != null)
                readPoint(endPoint.Value, offset, out vertices[^1]);
            ...
            static void readPoint(PointOrPathType value, Vector2 startPos, out PathControlPoint point)
            {
                if (value.IsPathType)
                    throw new ArgumentException("Must be a point instead of PathType", nameof(value));

                point = new PathControlPoint { Position = value.Point - startPos };
            }
