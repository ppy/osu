...
{
    /// <summary>
    /// A re-implementation of the "Combo Fire" effect from old versions of osu!stable.
    ...
    /// </summary>
    /// <remarks>
    /// This should be sized to fill the <see cref="Player"/>. Its draw height is managed internally.
    /// </remarks>
    public partial class LegacyComboFire : Drawable
    {
        ...
        /// <remarks>
        /// In practice the max alpha is achieved much earlier than this (at 89 combo) due to the clamp on
        /// <see cref="alphaFactor"/>.
        /// </remarks>
        private const int max_alpha_combo = 329;

        /// <remarks>
        /// This used to be a config option called <c>ComboFireHeight</c>. It was removed from the options menu early
        /// on, and the fire looks awful at higher values, so let's just keep it at the default (<c>3</c>).
        /// </remarks>
        ...
        private static readonly float height_scale = (Math.Max(0, (config_height - 5) / 10f) + 1) / 2;
        ...
        private readonly IBindable<int> comboBindable = new BindableInt();
        private readonly IBindable<bool> isBreakBindable = new BindableBool();
        private readonly IBindable<bool> showingOverlayBindable = new BindableBool();
        ...
        /// <summary>
        /// Separate internal alpha control used for fading fire in/out based on combo.
        /// </summary>
        ...
        public override DrawColourInfo DrawColourInfo =>
            base.DrawColourInfo with { Colour = base.DrawColourInfo.Colour.MultiplyAlpha(comboAlpha) };
        ...
        public LegacyComboFire(ScoreProcessor scoreProcessor)
        {
        ...
        private void load(ISkinSource skin, TextureStore textures, ShaderManager shaders)
        {
            if (!(skin.GetConfig<SkinConfiguration.LegacySetting, bool>(SkinConfiguration.LegacySetting.ComboFire)?.Value ?? false))
            {
                Expire();
                return;
            }

            shader = shaders.Load(@"LegacyComboFire", @"LegacyComboFire");
        ...
        protected override void LoadComplete()
        {
            ...
            comboBindable.BindTo(scoreProcessor.Combo);
            comboBindable.BindValueChanged(onComboChange, true);

            if (player != null)
            {
                isBreakBindable.BindTo(player.IsBreakTime);
                isBreakBindable.BindValueChanged(_ => onVisibilityChange(), true);

                showingOverlayBindable.BindTo(player.ShowingOverlayComponents);
                showingOverlayBindable.BindValueChanged(_ => onVisibilityChange(), true);
            }
        }
        ...
        {
            bool show = !isBreakBindable.Value && showingOverlayBindable.Value;

            this.FadeTo(show ? 1 : 0, HUDOverlay.FADE_DURATION, HUDOverlay.FADE_EASING);
        }
        ...
        {
            protected new LegacyComboFire Source => (LegacyComboFire)base.Source;
            ...
            public LegacyComboFireDrawNode(LegacyComboFire source)
                : base(source)
            {
            }
