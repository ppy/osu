...
using osuTK.Input;
using Sidebar = osu.Game.Screens.LLin.SideBar.Sidebar;

namespace osu.Game.Screens.LLin
{
    //todo: 重写界面?
    [Cached(typeof(IImplementLLin))]
    internal class OldMvisScreen : ScreenWithBeatmapBackground, IKeyBindingHandler<GlobalAction>, IImplementLLin
    {
        ...
        private void unBindFor(LLinPlugin pl)
        {
            //查找插件是pl的绑定
        ...
        private readonly BindableList<LLinPlugin> loadList = new BindableList<LLinPlugin>();
        ...
        private void onPluginUnLoad(LLinPlugin pl)
        {
            ...
            if ((LLinPlugin)currentFunctionBarProvider == pl)
        ...
        {
            if (sender is BindableList<LLinPlugin> list)
            {
                if (list.Count > 0)
