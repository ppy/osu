...
namespace osu.Game.Screens.Select
{
    internal class BeatmapInfoWedge : Container
    {
        ...
        {
            double bpmMax = beatmap.Timing.BPMMaximum; 
            double bpmMin = beatmap.Timing.BPMMinimum;

            if (Precision.AlmostEquals(bpmMin, bpmMax)) return Math.Round(bpmMin) + "bpm";

            return Math.Round(bpmMin) + "-" + Math.Round(bpmMax) + "bpm (mostly " + Math.Round(beatmap.Timing.BPMMode) + "bpm)";
        }
