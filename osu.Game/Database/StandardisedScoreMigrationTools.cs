    ...
    public static class StandardisedScoreMigrationTools
    {
        ...
        public static long GetNewStandardised(ScoreInfo score)
        {
            ...
            var maximumJudgements = score.MaximumStatistics
                                         .Where(kvp => kvp.Key.AffectsCombo())
                                         .OrderByDescending(kvp => processor.GetBaseScoreForResult(kvp.Key))
                                         .SelectMany(kvp => Enumerable.Repeat(new FakeJudgementInfo(kvp.Key), kvp.Value))
                                         .ToList();
            ...
            if (maximumJudgements.Count != sortedHits.Count)
            {
                maximumJudgements = sortedHits
                                    .Select(r => new FakeJudgementInfo(getMaxJudgementFor(r, maxRulesetJudgement)))
                                    .ToList();
            }
            ...
            foreach (var result in bonusHits)
                processor.ApplyResult(new Judgement(null!, new FakeJudgementInfo(result)) { Type = result });
            ...
            void insertMiss()
            {
                ...
                else
                {
                    ...
                    processor.ApplyResult(new Judgement(null!, new FakeJudgementInfo(getMaxJudgementFor(HitResult.Miss, maxRulesetJudgement)))
                    {
                        Type = HitResult.Miss,
                    });
                }
        ...
        {
            private readonly JudgementInfo judgementInfo;

            public override JudgementInfo CreateJudgement() => judgementInfo;

            public FakeHit(JudgementInfo judgementInfo)
            {
                this.judgementInfo = judgementInfo;
            }
        }

        private class FakeJudgementInfo : JudgementInfo
        {
            public override HitResult MaxResult { get; }

            public FakeJudgementInfo(HitResult maxResult)
            {
                MaxResult = maxResult;
            }
        }
