...
#pragma warning restore 618
        ...
        private void flushPendingPoints()
        {
            ...
            // As non-timing-points should not set the OmitFirstBarline setting, we will honor the setting of the last effect point, the effect point generated by the timing-point (if any).
            EffectControlPoint effectPoint = null;

            for (int i = pendingControlPoints.Count - 1; i >= 0; i--)
            {
                if (pendingControlPoints[i] is EffectControlPoint pendingEffectPoint)
                {
                    if (effectPoint == null)
                        effectPoint = pendingEffectPoint;
                    else
                        effectPoint.OmitFirstBarLine = pendingEffectPoint.OmitFirstBarLine;
                }
                else
                {
            ...
            if (effectPoint != null)
                beatmap.ControlPointInfo.Add(pendingControlPointsTime, effectPoint);
