...
namespace osu.Game.Beatmaps
{
    ...
    public class BeatmapStore : DatabaseBackedStore
    {
        ...
        public bool Delete(BeatmapSetInfo beatmapSet)
        {
            using (var context = GetContext())
            {
                Refresh(context, ref beatmapSet);
        ...
        public IQueryable<BeatmapSetInfo> BeatmapSets
        {
            get
            {
                // TODO Dispose this context. Can't dispose it right here since it will break external query logic (for example, Query methods in the BeatmapManager)
                return GetContext().BeatmapSetInfo
                                   .Include(s => s.Metadata)
